...
export class InviteBuddy extends Component {
    ...
    componentDidMount() {
        AXIOS.get("http://localhost:4000/friend/" + JSON.parse(localStorage.getItem('user'))._id)
            .then(result => {
                console.log(result.data.friend.confirmed_friends);

                this.setState({ friends: this.showFriends(result.data.friend.confirmed_friends) });
            });
    ...
    showFriends(list) {
        ...
        for (let i = 0; i < list.length; i++) {
            AXIOS.get("http://localhost:4000/user/" + list[i])
                .then(response => {
        ...
        return (
            <Modal
                ...
                <Modal.Body>
                    ...
                    <div>{this.state.invitation_boolean ? (
                        <Alert
