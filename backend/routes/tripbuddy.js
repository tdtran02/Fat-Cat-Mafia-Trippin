...
TRIPBUDDYROUTES.route("/buddy").post(function (req, res) {
  TRIPBUDDY.findOne({ trip_id: req.body.trip_id, buddy_id: req.body.buddy_id })
    .then((tripbuddy) => {
      if (tripbuddy != null) {
        res.status(200).json({
        ...
        B.save()
          .then((x) => {
            TRIP.findByIdAndUpdate(req.body.trip_id, {
              $addToset: { buddies: req.body.buddy_id },
            }).then((r) => {
              res.status(200).json({
                saved: true,
...
TRIPBUDDYROUTES.route("/buddy/:trip_id").get(function (req, res) {
  TRIPBUDDY.find({ trip_id: req.params.trip_id }).then((tripbuddy) => {
    if (tripbuddy != null) {
      res.status(200).json({
        tripbuddy: tripbuddy,
      });
...
TRIPBUDDYROUTES.route("/buddypending/:buddy_id").get(function (req, res) {
  TRIPBUDDY.find({ buddy_id: req.params.buddy_id }).then((tripbuddy) => {
    if (tripbuddy != null) {
      res.status(200).json({
        tripbuddy: tripbuddy,
      });
...
TRIPBUDDYROUTES.route("/buddy/:id").delete(function (req, res) {
  TRIPBUDDY.deleteMany({ trip_id: req.params.id }).then((tripbuddy) => {
    if (tripbuddy != null) {
      res.status(200).json({
